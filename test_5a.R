...
outfile = sprintf('Results/Raw/test_5a_run_%03d.csv',runID)
...
E_validation_randn = matrix(rnorm(n_validation*2),n_validation,2) # new errors for training clones
  ...
  foreach(cor_R = c(-0.9,-0.6,-0.4,-0.2,0,0.2,0.4,0.6,0.9),.combine = 'rbind',.errorhandling = 'remove') %dopar% {
    foreach(H21 = c(0.2,.6),.combine = 'rbind',.errorhandling = 'remove') %do% { #0.1,0.4,0.7
      foreach(H22 = c(0.2,0.6),.combine = 'rbind',.errorhandling = 'remove') %do% { #0.4,
        ...
        # adjust H2s for validation individuals (clones have 2x residual variance of training lines)
        H2s_adj = diag(G)/(diag(G)+2*diag(R))
        ...
        Y_o = as.matrix(U[training,] + E[training,])
        U_n = U[validation,]
        Y_n = U_n + E[validation,]*sqrt(2) # validation clones for trait 2 - note: s2_E is larger because these have 1/2 the sample size
        Y_n_clone = U_n + E_validation_randn*sqrt(2)
        ...
        V_c = Ghat[2,2]*K_c + 2*Rhat[2,2]*I_nn  # note: 2x residual variance estimate
        uhat_CV2 = as.matrix(uhat_CV1[,1] + (Ghat[1,2]*K_c) %*% solve(V_c,Y_n[,2]-fixef(m2)[2] - uhat_CV1[,2]))

        # calculate correlations
        results = data.frame(
          Single_u = cor(uhat_single,U_n[,1]),
          Single_y = cor(uhat_single,Y_n_clone[,1]),
          CV1_u = cor(uhat_CV1[,1],U_n[,1]),
          CV1_y = cor(uhat_CV1[,1],Y_n_clone[,1]),
          CV2_u = cor(uhat_CV2,U_n[,1]),
          CV2_y = cor(uhat_CV2,Y_n_clone[,1])
        )
        ...
        results = data.frame(runID,Rep,
                             ...
                             H2s_adj = matrix(H2s_adj[1:2],nr=1),
                             results)
