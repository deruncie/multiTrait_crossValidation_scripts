...
outfile = sprintf('Results/Raw/test_3a_run_%03d.csv',runID)
if(file.exists(outfile)) quit('no')

# Select training, validation sets
n_validation = round(.5*n)
  ...
  foreach(cor_R = c(-0.9,-0.6,-0.4,-0.2,0,0.2,0.4,0.6,0.9),.combine = 'rbind',.errorhandling = 'remove') %dopar% {
    foreach(H21 = c(0.2,.6),.combine = 'rbind',.errorhandling = 'remove') %do% { #0.1,0.4,0.7
      foreach(H22 = c(0.2,0.6),.combine = 'rbind',.errorhandling = 'remove') %do% { #0.4,
        ...
        # estimate Genetic correlation in training data of each estimate
        Y_predictions = cbind(Y_n[,1],uhat_single)
        Y_predictions = sweep(Y_predictions,2,sqrt(diag(cov(Y_predictions))),'/')
        m1_validation = relmatLmer(c(t(Q_nn) %*%Y_predictions)~0+X_validation+(0+Trait|Line)+(0+Trait|ID),
                                   tall_data_validation,relmat = list(Line = D_K_nn))
        var1_validation = as.data.frame(VarCorr(m1_validation))
        h2_uhat_Single = var1_validation$vcov[2]/sum(var1_validation$vcov[c(2,5,7)])
        # gcor_y_Yhat_Single = var1_validation$vcov[3]#/sqrt(var1_validation$vcov[1]*var1_validation$vcov[2])

        Y_predictions = cbind(Y_n[,1],uhat_CV1[,1])
        m2_validation = relmatLmer(c(t(Q_nn) %*%Y_predictions)~0+X_validation+(0+Trait|Line)+(0+Trait|ID),
                                   tall_data_validation,relmat = list(Line = D_K_nn))
        var2_validation = as.data.frame(VarCorr(m2_validation))
        h2_uhat_CV1 = var2_validation$vcov[2]/sum(var2_validation$vcov[c(2,5,7)])
        # gcor_y_Yhat_CV1 = var2_validation$vcov[3]#/sqrt(var2_validation$vcov[1]*var2_validation$vcov[2])


        Y_predictions = cbind(Y_n[,1],uhat_CV2)
        m2b_validation = relmatLmer(c(t(Q_nn) %*%Y_predictions)~0+X_validation+(0+Trait|Line)+(0+Trait|ID),
                                    tall_data_validation,relmat = list(Line = D_K_nn))
        var2b_validation = as.data.frame(VarCorr(m2b_validation))
        h2_uhat_CV2 = var2b_validation$vcov[2]/sum(var2b_validation$vcov[c(2,5,7)])
        # gcor_y_Yhat_CV2 = var2b_validation$vcov[3]#/sqrt(var2b_validation$vcov[1]*var2b_validation$vcov[2])


        # calculate correlations
        results = data.frame(
          Single_gcor_u = var1_validation$vcov[3]/sqrt(var1_validation$vcov[1]*var1_validation$vcov[2]) * sqrt(h2_uhat_Single),
          CV1_gcor_u = var2_validation$vcov[3]/sqrt(var2_validation$vcov[1]*var2_validation$vcov[2]) * sqrt(h2_uhat_CV1),
          CV2_gcor_u = var2b_validation$vcov[3]/sqrt(var2b_validation$vcov[1]*var2b_validation$vcov[2]) * sqrt(h2_uhat_CV2)
        )
