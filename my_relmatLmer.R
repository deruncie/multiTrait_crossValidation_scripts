...
)
{
  ...
  # recover()
  for(i in seq_along(lmod$reTrms)) {
    fnmn = fnmns[i]
    if(fnmn %in% relnms) {
      Ki = relmat[[fnmn]]
      if(!all(levels(flist[[fnmn]]) %in% rownames(Ki))) stop(sprintf("Levels of %s missing from relmat",fmnm))
      Zti = lmod$reTrms$Ztlist[[i]]
      Ki = Ki[levels(flist[[fnmn]]),levels(flist[[fnmn]])]
      Kisq_t = relfac(Ki, method.relfac)
      relfac[[fnmn]] = Kisq_t
      ng = length(lmod$reTrms$cnms[[i]])
      for(j in 1:ng) {
        Zti[seq(j,nrow(Zti),by=ng),] = Kisq_t %*% Zti[seq(j,nrow(Zti),by=ng),]
      }
      #
      # Ki = Matrix(K)[rownames(Zti),rownames(Zti)]
      # Kisq_t = relfac(Ki, method.relfac)
      # # Kisq_t = Matrix(Ksq_t)[rownames(Zti),rownames(Zti)]#/sqrt(2)
      # Zti = Kisq_t %*% Zti
      lmod$reTrms$Ztlist[[i]] = Zti
    }
  }
