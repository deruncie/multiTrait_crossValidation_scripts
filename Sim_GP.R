...
outfile = sprintf('Results_1level/run_%dLines_%03d.csv',n_Line,runID)
...
# library(data.table)
# library(doParallel)
# files = list.files(path='Results_1level',pattern='.csv',full.names=T)
# registerDoParallel(10)
# results = foreach(file = files,.combine = 'rbind') %dopar% {
#   print(match(file,files)/length(files))
#   fread(file,data.table=F)
# }
# saveRDS(results,file = 'Results_1level/collected_results_1level.rds')
