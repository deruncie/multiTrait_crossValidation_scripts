...
outfile = sprintf('Results/Raw/test_5b_run_%03d.csv',runID)
...
validation_pairs = c()   # for each validation individual, also hold out it's nearest non-validation relative. Will use this individual as the test.
validation_pairs_k = c()
for(i in 1:length(validation)) {
  o = order(K[validation[i],training],decreasing = T)
  validation_pairs[i] = training[o[1]]
  validation_pairs_k[i] = K[validation[i],validation_pairs[i]]
  training = training[-o[1]]
}
# r = c(r,mean(validation_pairs_k))
# }
...
#------------------------------------------------------------#
{
  ...
  K_xx = K[validation_pairs,validation_pairs]
  K_xo = K[validation_pairs,training]
  K_xn = K[validation_pairs,validation]
  K_cx = K_xx - K_xo %*% solve(K_oo,t(K_xo))
  ...
  foreach(cor_R = c(-0.9,-0.6,-0.4,-0.2,0,0.2,0.4,0.6,0.9),.combine = 'rbind',.errorhandling = 'remove') %dopar% {
    foreach(H21 = c(0.2,.6),.combine = 'rbind',.errorhandling = 'remove') %do% { #0.1,0.4,0.7
      foreach(H22 = c(0.2,0.6),.combine = 'rbind',.errorhandling = 'remove') %do% { #0.4,
        ...
        U_n = U[validation,]
        Y_n = Y[validation,]
        U_relatives = U[validation_pairs,]
        Y_relatives = Y[validation_pairs,]
        ...
        # uhat_CV2 = as.matrix(uhat_CV1[,1] + (Ghat[1,2]*K_c) %*% solve(V_c,Y_relatives[,2]-fixef(m2)[2] - uhat_CV1[,2]))
        # Form covariance [A B;Bt C] for (u_n1, y_x1, u_01,u_o2)
        # from this, calculate Sigma = conditional covariance of (u_n1, y_x1) | u_o
        # from this get conditional mean of u_n1 | y_x1, u_o
        A = rbind(cbind(Ghat[1,1]*K_nn,kronecker(Ghat[1,2],t(K_xn))),cbind(kronecker(Ghat[2,1],K_xn),kronecker(Ghat[2,2],K_xx) + kronecker(Rhat[2,2],I_nn)))
        B = rbind(cbind(Ghat[1,1]*K_no,kronecker(Ghat[1,2],K_no)),cbind(Ghat[1,1]*K_xo,kronecker(Ghat[1,2],K_xo)))
        Cinv = kronecker(solve(Ghat),K_oo_inv)
        Sigma = A - B %*% Cinv %*% t(B)
        Sigma_11 = Sigma[1:n_validation,1:n_validation]
        Sigma_12 = Sigma[1:n_validation,n_validation+1:n_validation]
        Sigma_22 = Sigma[n_validation+1:n_validation,n_validation+1:n_validation]

        uhat_CV1_relatives = K_xo %*% K_oo_inv %*% uhat_o
        uhat_CV2 = as.matrix(uhat_CV1[,1] + Sigma_12 %*% solve(Sigma_22,Y_relatives[,2]-fixef(m2)[2] - uhat_CV1_relatives[,2]))
